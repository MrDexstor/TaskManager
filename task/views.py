...
from api.task import getCorrectionsTask
...
def action(request, id, actions):
    task = Task.objects.get(id=id)
    task.state = task_state.objects.get(id=actions)
    task.save(update_fields=["state"])
    return redirect(f'/task/base/{id}')


def corrections_task(request):
    return render(request, 'task/corrections_task.html', getCorrectionsTask(request))
